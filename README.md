...
- 支持的调试命令：
  - `help`: 显示帮助信息
  - `info <编号>`: 查询指定蛇的信息（包括待转向方向）
  - `kill <编号>`: 强制杀死指定蛇
  - `respawn <编号>`: 原地复活已死亡的蛇（如果位置被占用则失败）
  - `edit <编号> l <值>`: 调整蛇的长度（考虑待增加长度）
  - `edit <编号> d <方向>`: 设置蛇的强制转向（支持 u/d/l/r 或数字）
  - `continue`: 退出调试控制台
  - `exit`: 结束程序
...
- 所有用户输入都经过验证（如蛇编号、方向值等）
- 使用 try-catch 捕获异常并提供友好错误信息
- 边界检查防止数组越界访问
...
- 使用 RAII 模式管理 ncurses 资源（NcuInit 类）
- 智能指针（std::unique_ptr）管理动态分配的对象
